...
def main(limit=None, offset=0, minimum_match_value=0.8):
    ...
    raw_parsed_locations = alchemy_session.query(alchemy.grant.RawLocation).limit(limit).offset(offset)
    ...
    unique_group_count = alchemy_session.query(expression.func.count(sqlalchemy.distinct(alchemy.grant.Location.id))).all()
...
def run_geo_match(key, default, match_group, counter, runtime):
    ...
    if len(match_group) > 1:
        ...
        clean = alchemy_session.query(alchemy.grant.Location).filter(alchemy.grant.Location.id == key).first()
        if clean:
            param = clean.summarize
            param.pop("id")
            param.pop("latitude")
            param.pop("longitude")
            loc = alchemy_session.query(alchemy.grant.RawLocation)\
                .filter(alchemy.grant.RawLocation.city == param["city"])\
                .filter(alchemy.grant.RawLocation.state == param["state"])\
                .filter(alchemy.grant.RawLocation.country == param["country"])\
                .first()
            if loc:
